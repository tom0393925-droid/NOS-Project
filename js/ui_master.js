...
function saveSingleMaster() {
    ...
    skuMaster[code] = {
        ...
        weight: parseFloat(document.getElementById('mWeight').value) || 0.0,
        safetyStock: parseInt(document.getElementById('mSafety').value) || 0,
        storageType: document.getElementById('mStorage').value,
        manufacture: document.getElementById('mManufacture') ? document.getElementById('mManufacture').value : '',
...
function renderMasterList() {
    ...
    for (const code in skuMaster) {
        ...
        let mfBadge = '';
        if (item.manufacture === 'JP Frozen') mfBadge = '<span class="bg-red-50 border border-red-200 text-red-700 px-2 py-0.5 rounded text-[10px] font-bold">🇯🇵 JP</span>';
        else if (item.manufacture === 'Container') mfBadge = '<span class="bg-blue-50 border border-blue-200 text-blue-700 px-2 py-0.5 rounded text-[10px] font-bold">🚢 CTN</span>';
