...
| `ui_sku_insights.js` | SKU Insightsタブ。SKUを選ぶと、そのSKUの売上（金額／数量）を固定4週ごとの棒グラフで表示。一番右の進行中の期間は「着地見込み（est.）」として満額に引き伸ばして表示する。下部にはそのSKUを発注している顧客の一覧（累計・発注間隔・最終発注・経過週数）を表示。「Compare」モードでは複数SKUを選んで積み上げ棒グラフで合計を比較できる |
